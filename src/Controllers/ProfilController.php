...
if (isset($_POST['email']) && isset($_POST['emailretype'])) {
    ...
    if ($email === $emailretype) {
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($conn->query($sql)) {
                ...
                header("Location: ../views/profil.php?modify=email&error=success");
            } else echo 'nnoon';
        } else header("Location: ../views/profil.php?modify=email&error=emailnotvalid");
    } else header("Location: ../views/profil.php?modify=email&error=emailnotsame");
    ...
    if ($password === $resultoldpassword['password']) {
        if ($newpassword === $passwordretype) {
            ...
            if ($conn->query($sql)) {
                header("Location: ../views/profil.php?modify=password&error=success");
            }
        } else header("Location: ../views/profil.php?modify=password&error=passnotsame");
    } else header("Location: ../views/profil.php?modify=password&error=oldpasserror");
}
