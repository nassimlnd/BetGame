...
if (isset($_POST['mise'])) {
    BetController::miseBet($conn);
}

class BetController
{
    public static function checkBet(): void
    {
        $conn = Database::databaseConnect();
            ...
            for ($i = 0; $i < count($arrayallbets); $i++) {
                if ($arrayallbets[$i]['validated'] == 0) {
                                ...
                                if ($matchidbetdetails == $arrayjson['response'][$k]['fixture']['id']) {
                                    if ($arrayjson['response'][$k]['fixture']['status']['long'] == "Game Finished") {
                                        ...
                                        if ($betbetdetails == $winner) {
                                            ...
                                            $conn->query("UPDATE bets SET status = 0, validated = 1 WHERE id =" . $arrayallbets[$i]['id']);
                                            $loose = true;
                                        }
                                    } else break;
                                }
                                ...
                                if ($matchidbetdetails == $arrayjson['response'][$k]['id']) {
                                    if ($arrayjson['response'][$k]['status']['long'] == "Game Finished") {
                                        ...
                                        if ($betbetdetails == $winner) {
                                            ...
                                            $basketQuery = "UPDATE bets SET status = 0 WHERE id =" . $arrayallbets[$i]['id'];
                                            $basketQuery2 = "UPDATE bets SET validated = 1 WHERE id =" . $arrayallbets[$i]['id'];
                                            $conn->query($basketQuery) && $conn->query($basketQuery2);
                                            $loose = true;
                                        }
                                    } else break;
                                }
                    ...
                    if ($win == true && $loose == false) {
                        ...
                        $conn->query("UPDATE bets SET status = 1 WHERE id =" . $arrayallbets[$i]['id']) && $conn->query("UPDATE bets SET validated = 1 WHERE id =" . $arrayallbets[$i]['id']);
                        PointsController::setPoints($newamountpoints, $conn);
                    }
                }
            }
        }
    ...
    public static function addBet(): void
    {
        $conn = Database::databaseConnect();
    ...
    public static function miseBet(): void
    {
        $conn = Database::databaseConnect();
    ...
    public static function deleteBet(): void
    {
        if (!isset($_SESSION['user'])) {
            session_start();
        }
